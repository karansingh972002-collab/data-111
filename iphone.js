...
function setHeroImage(button) {
  ...
  if (!image || !button?.dataset.gallerySrc) return;
  image.src = button.dataset.gallerySrc;
  image.alt = button.dataset.galleryAlt || button.dataset.galleryColor || "iPhone product photo";
}
...
function setColor(color, tone) {
  ...
  setHeroImage(document.querySelector(`[data-gallery-color="${color}"]`));
  updateSummary();
}
